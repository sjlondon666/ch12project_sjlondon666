...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   =
    $sql = "SELECT full_name, email FROM tbl_student WHERE email = sjlondon666@mail.cfcc.edu' '$email'";
    $result = mysqli_query($conn, $sql);

    if (mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        echo "<div class='message'>Student found: " . $row['full_name'] . " (" . $row['email'] . ")</div>";
    } else {
